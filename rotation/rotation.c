...
BOOL initRotationEngine(UBYTE rs, USHORT bw, USHORT bh) {
    ...
    y_mult_sin = AllocVec(sizeof(int) * MAX_BITMAP_HEIGHT, MEMF_FAST | MEMF_CLEAR);
    if (!y_mult_sin) {
        writeLog("Error: Could not allocate memory for y_mult_sin\n");
        return FALSE;
    }

    y_mult_cos = AllocVec(sizeof(int) * MAX_BITMAP_HEIGHT, MEMF_FAST | MEMF_CLEAR);
    if (!y_mult_cos) {
        writeLog("Error: Could not allocate memory for y_mult_cos\n");
        return FALSE;
    }

    x_mult_sin = AllocVec(sizeof(int) * MAX_BITMAP_WIDTH, MEMF_FAST | MEMF_CLEAR);
    if (!x_mult_sin) {
        writeLog("Error: Could not allocate memory for x_mult_sin\n");
        return FALSE;
    }

    x_mult_cos = AllocVec(sizeof(int) * MAX_BITMAP_WIDTH, MEMF_FAST | MEMF_CLEAR);
    if (!x_mult_cos) {
